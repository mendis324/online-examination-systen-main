...
<div class="row">
<div class="col-lg-6">
<span class="logo">Check Your Knowdledge</span></div>
...
<nav class="navbar navbar-default title1">
  <div class="container-fluid">
    ...
    <li><a href="feedback.php" target="_blank">Feedback</a></div></div></li>
		</ul>
      </div><!-- /.navbar-collapse -->
...
//ranking start
if (@$_GET['q'] == 3) {
    // Fetch the result set ordered by score
    ...
    // Loop through each row in the result set to find the rank of the current user
    while ($row1 = mysqli_fetch_assoc($q)) {
        // Check if the email matches the one stored in $email
        if ($row1['email'] == $email) {
          if($row_number ==1){
            ?><h2><b>Congratulations</b></h2><?php
          }
          ?>
          <h3><b>Your rank is <?php echo $row_number; ?></b></h3>
          <?php
            $found = true;
            break;  // Exit the loop once the email is found
        }
        // Increment the row number for the next record
        $row_number++;
    }

    if (!$found) {
        echo "You Did Not Complete Any Quizzes";
    }

    // Reset the result pointer to the beginning of the result set
    mysqli_data_seek($q, 0);  // Reset the result set pointer to the start
    ...
    // Loop through the result set again to display all records with rank, name, college, and score
    while ($row = mysqli_fetch_array($q)) {
        $e = $row['email'];
        $s = $row['score'];

        // Fetch the user details from the "users" table based on the email
        $q12 = mysqli_query($con, "SELECT * FROM users WHERE email='$e'") or die('Error231');

        while ($row_user = mysqli_fetch_array($q12)) {
            $name = $row_user['name'];
            $college = $row_user['collage'];  // Assuming it's spelled "collage" in your database
        }
